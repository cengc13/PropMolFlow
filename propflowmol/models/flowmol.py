...
from propflowmol.models.lr_scheduler import LRScheduler
from propflowmol.models.interpolant_scheduler import InterpolantScheduler
from propflowmol.models.vector_field import EndpointVectorField, VectorField, DirichletVectorField
from propflowmol.models.ctmc_vector_field import CTMCVectorField

from propflowmol.data_processing.utils import build_edge_idxs, get_upper_edge_mask, get_batch_idxs
from propflowmol.data_processing.priors import uniform_simplex_prior, biased_simplex_prior, batched_rigid_alignment, rigid_alignment
from propflowmol.data_processing.priors import inference_prior_register, edge_prior
from propflowmol.analysis.molecule_builder import SampledMolecule
from propflowmol.analysis.metrics import SampleAnalyzer
...
class FlowMol(pl.LightningModule):
    ...
    def sample_random_sizes(self, n_molecules: int, device="cuda:0",
        ...
        property_name:str=None, properties_for_sampling:int|float=None, # properties name is for finding normalizing vector for the property
...
# ###############################
#     # add auto-guidance part
#     @torch.no_grad()
#     def sample_with_guidance(self, n_atoms: torch.Tensor, guide_model, n_timesteps: int = None, 
#                             device="cuda:0", guide_w={'x': 2.0, 'a': 1.0, 'c': 1.0, 'e': 1.0},
#                             stochasticity=None, high_confidence_threshold=None, 
#                             xt_traj=False, ep_traj=False, method='midpoint', normalization_file_path:str=None,
#                             conditional_generation:bool=True, property_name:str=None, properties_for_sampling:int|float=None,
#                             **kwargs):
#         """Sample molecules with auto-guidance from a better model.
        
#         Args:
#             n_atoms (torch.Tensor): Tensor of shape (batch_size,) containing number of atoms per molecule
#             guide_model: Another instance of FlowMol with better performance
#             guide_w: Weight for guidance, higher values follow guide_model more closely
#             **kwargs: Additional arguments passed to integration
#         """
#         if n_timesteps is None:
#             n_timesteps = self.default_n_timesteps
...
#         # Add guidance to vector field if not already done
#         if not hasattr(self.vector_field, 'good_model'):
#             self.vector_field.add_auto_guidance(guide_model.vector_field)

#         # Build edge indices dictionary
#         edge_idxs_dict = {}
#         for n_atoms_i in torch.unique(n_atoms):
#             edge_idxs_dict[int(n_atoms_i)] = build_edge_idxs(n_atoms_i)

#         # Construct graph for each molecule
#         g = []
#         for n_atoms_i in n_atoms:
#             edge_idxs = edge_idxs_dict[int(n_atoms_i)]
#             g_i = dgl.graph((edge_idxs[0], edge_idxs[1]), num_nodes=n_atoms_i, device=device)
...
#         itg_result = self.vector_field.integrate_with_guidance(g, node_batch_idx, **integrate_kwargs, **kwargs)
...
#     def sample_random_sizes_with_guidance(self, n_molecules: int, guide_model, device="cuda:0",
#                                         guide_w={'x': 2.0, 'a': 1.0, 'c': 1.0, 'e': 1.0},
#                                         stochasticity=None, high_confidence_threshold=None,
#                                         xt_traj=False, ep_traj=False, method='midpoint',
#                                         normalization_file_path:str=None, conditional_generation:bool=True,
#                                         property_name:str=None, properties_for_sampling:int|float=None,
#                                          **kwargs):
                                        
#         """Sample n_molecules with auto-guidance, using random sizes from training distribution."""
        
#         # Sample number of atoms per molecule
#         atoms_per_molecule = self.sample_n_atoms(n_molecules).to(device)

#         # Sample with guidance
#         return self.sample_with_guidance(atoms_per_molecule,
