...
from propflowmol.data_processing.dataset import collate
from propflowmol.data_processing.dataset import MoleculeDataset
from propflowmol.data_processing.samplers import SameSizeMoleculeSampler, SameSizeDistributedMoleculeSampler
...
class MoleculeDataModule(pl.LightningDataModule):
    ...
    def train_dataloader(self):
        ...
        if self.x_subspace == 'se3-quotient':
            ...
            dataloader = DataLoader(dataset=self.train_dataset, batch_sampler=batch_sampler, collate_fn=collate, num_workers=self.num_workers)
    ...
    def val_dataloader(self):
        ...
        if self.x_subspace == 'se3-quotient':
            # if we are using the se3-quotient subspace, then we need to do same-size sampling so that we can efficiently compute rigid aligments during training
            if self.distributed:
                batch_sampler = SameSizeDistributedMoleculeSampler(self.val_dataset, batch_size=self.batch_size*2)
            else:
                batch_sampler = SameSizeMoleculeSampler(self.val_dataset, batch_size=self.batch_size*2)

            dataloader = DataLoader(dataset=self.val_dataset, batch_sampler=batch_sampler, collate_fn=collate, num_workers=self.num_workers)
